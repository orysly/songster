...
export function PlaylistSummary({ playlist }: { playlist: PlaylistMeta | null }) {
  ...
  return (
    <div className="rounded-2xl bg-white/10 p-4 text-white ring-1 ring-white/20">
      <p className="text-sm uppercase tracking-wide text-white/55">Playlist loaded</p>
      ...
      {playlist.usableCount < 10 ? (
        <p className="mt-3 text-sm text-white/55">Load a playlist with at least 10 usable songs to start.</p>
      ) : null}
    </div>
  );
}
